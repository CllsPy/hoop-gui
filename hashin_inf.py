...
class Hashin_Inf(Micmodelos):
    def __init__(self):
        super().__init__()
        self.tipo = 'Hashin - Inferior'
    ...
    def calc(self, objH): #<------- FUNCIONANDO =======================
        if objH.analH.analise_type == "mecanica":
            ...
            Eo = 9 * Ko * Go / (3 * Ko + Go)
            self.Eh = 9 * Kf * Gf / (3 * Kf + Gf)
            vo = (3 * Ko - 2 * Go) / (2 * (3 * Ko + Go))
            self.vh = (3 * Kf - 2 * Gf) / (2 * (3 * Kf + Gf))
            ...
            kh = (Km@Ki + 2*Km@(Km*fm+Ki*fi))/(2*Km+Km*fi+Ki*fm)
